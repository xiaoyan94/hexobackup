...
{% post_link "备份天天在跑、恢复点却停在33天前：XtraBackup增量备份脚本全面翻修实录" "上一篇文章" %}复盘了一次备份链静默断裂 33 天的事故，并给出了翻修后的自动备份脚本全文。但复盘解决的是"为什么这么写"，真正落地时还有一整串问题：生产环境是**离线的 CentOS 7**，装不了公网仓库；XtraBackup 必须和 MySQL 5.7 严格配套；压缩备份的解压依赖 qpress；脚本在 Windows 上编辑过带着 CRLF；定时任务怎么和已有的维护窗口错峰……本文是姊妹篇，从一台裸机开始，把整套物理备份体系装起来：**RPM 离线安装 → qpress → 脚本部署与配置 → 钉钉告警 → 首跑验证 → 定时任务 → 恢复演练**，照着走一遍，半小时落地。
